...
'ContentTopLevel_ContentPlaceHolder1_hlTournamentTeam','ContentTopLevel_ContentPlaceHolder1_lblBestPGGrade', 'ContentTopLevel_ContentPlaceHolder1_lblPos', 'ContentTopLevel_ContentPlaceHolder1_lblHSGrad',
 'ContentTopLevel_ContentPlaceHolder1_lblBT'];

eventSelectors = ['ContentTopLevel_ContentPlaceHolder1_rptEvents_lblFB_', 'ContentTopLevel_ContentPlaceHolder1_rptEvents_lblRange_', 'ContentTopLevel_ContentPlaceHolder1_rptEvents_lblCB_' , 'ContentTopLevel_ContentPlaceHolder1_rptEvents_lblCH_'
, 'ContentTopLevel_ContentPlaceHolder1_rptEvents_lblSL_', 'ContentTopLevel_ContentPlaceHolder1_rptEvents_lbl60_', 'ContentTopLevel_ContentPlaceHolder1_rptEvents_lbl10_', 'ContentTopLevel_ContentPlaceHolder1_rptEvents_lblIF_'];
...
function setupCSV() {
    ...
    stream.write('Name, High School, College, Latest Travel Team, Latest PG Grade, Positions, Graduation Year, Bat/Throws, ');

    for (var x = 1; x < 100; x++) {
        stream.write(`Event ${x} FB, Event ${x} FB-Range, Event ${x} CB, Event ${x} CH, Event ${x} SL, Event ${x} 60, Event ${x} 10, Event ${x} IF, `);
    }
    stream.write('\n');
    stream.end();
});
    ...
    (async () => {
        ...
        for (var xs = 0; xs < pLinkArray.length; xs++) {
            try {
                ...
                await page.click('[id="ContentTopLevel_ContentPlaceHolder1_liEvents"]', { waitUntil:'domcontentloaded' });
                await new Promise(r => setTimeout(r, 1000));
